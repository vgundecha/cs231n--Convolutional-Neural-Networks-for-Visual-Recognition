...
#%% Generate a class visualization

target_y = 46 # Tarantula
...
def deepdream(X, layer, model, **kwargs):
  ...
  for t in xrange(num_iterations):
    ...
    # update the image X.                                                      #
    ############################################################################
    activations, cache = model.forward(X, end=layer)
    dout = activations
    dX, _ = model.backward(dout, cache)
    X += learning_rate*dX
...
out = deepdream(img_pre, 7, model, learning_rate=2000, num_iterations=200)  